...
import { FiPlus, FiEdit2, FiTrash2, FiSave, FiX, FiChevronRight, FiChevronDown } from 'react-icons/fi';
        ...
        <div className="card" style={{ marginBottom: 'var(--space-6)', padding: 'var(--space-6)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
            <div>
              ...
              <input className="form-input" value={lessonTitle} onChange={e => setLessonTitle(e.target.value)} placeholder="VD: Chào hỏi cơ bản" />
            </div>
            <div>
              <label className="form-label">Thứ tự</label>
              <input className="form-input" type="number" value={lessonOrder} onChange={e => setLessonOrder(e.target.value)} />
            </div>
            <div style={{ gridColumn: 'span 2' }}>
              <label className="form-label">Mô tả</label>
              <textarea className="form-input" value={lessonDesc} onChange={e => setLessonDesc(e.target.value)} rows={2} />
            </div>
          </div>
            ...
            {selectedLesson?.Id === lesson.Id && (
              <div style={{ padding: 'var(--space-4)', borderTop: '1px solid var(--color-border)', background: 'rgba(0,0,0,0.01)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-4)' }}>
                  ...
                  <button className="btn btn-primary btn-sm" onClick={() => setShowQuestionForm(true)}><FiPlus /> Thêm câu hỏi</button>
                </div>
                ...
                {showQuestionForm && (
                  <div className="card" style={{ marginBottom: 'var(--space-4)', padding: 'var(--space-4)', background: 'white' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 'var(--space-3)' }}>
                      <div style={{ gridColumn: 'span 2' }}>
                        <label className="form-label">Câu hỏi (English)</label>
                        <input className="form-input" value={questionText} onChange={e => setQuestionText(e.target.value)} />
                      </div>
                      <div style={{ gridColumn: 'span 2' }}>
                        <label className="form-label">Dịch (Tiếng Việt)</label>
                        <input className="form-input" value={questionTrans} onChange={e => setQuestionTrans(e.target.value)} />
                      </div>
                      
                      {/* Options */}
                      <div>
                        <label className="form-label">Option 1 (EN)</label>
                        <input className="form-input form-input-sm" value={opt1} onChange={e => setOpt1(e.target.value)} />
                      </div>
                      <div>
                        <label className="form-label">Dịch Option 1 (VI)</label>
                        <input className="form-input form-input-sm" value={opt1vi} onChange={e => setOpt1vi(e.target.value)} />
                      </div>
                      <div>
                        <label className="form-label">Option 2 (EN)</label>
                        <input className="form-input form-input-sm" value={opt2} onChange={e => setOpt2(e.target.value)} />
                      </div>
                      <div>
                        <label className="form-label">Dịch Option 2 (VI)</label>
                        <input className="form-input form-input-sm" value={opt2vi} onChange={e => setOpt2vi(e.target.value)} />
                      </div>
                      <div>
                        <label className="form-label">Option 3 (EN)</label>
                        <input className="form-input form-input-sm" value={opt3} onChange={e => setOpt3(e.target.value)} />
                      </div>
                      <div>
                        <label className="form-label">Dịch Option 3 (VI)</label>
                        <input className="form-input form-input-sm" value={opt3vi} onChange={e => setOpt3vi(e.target.value)} />
                      </div>
                      <div>
                        <label className="form-label">Thứ tự</label>
                        <input className="form-input form-input-sm" type="number" value={qOrder} onChange={e => setQOrder(e.target.value)} />
                      </div>
                    </div>
                    <div style={{ marginTop: 'var(--space-3)', display: 'flex', gap: 'var(--space-2)' }}>
                      <button className="btn btn-primary btn-sm" onClick={handleSaveQuestion}><FiSave /> Lưu</button>
                      <button className="btn btn-ghost btn-sm" onClick={closeQuestionForm}><FiX /> Hủy</button>
                    </div>
                  </div>
                )}
                  ...
                  {questions.map(q => (
                    <div key={q.Id} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', background: 'white', padding: 'var(--space-3)', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)' }}>
                      ...
                      <div style={{ display: 'flex', gap: 'var(--space-1)' }}>
                        ...
                        <button className="btn btn-ghost btn-xs" style={{ color: 'var(--color-error)' }} onClick={() => handleDeleteQuestion(q.Id)}><FiTrash2 size={12} /></button>
                      </div>
                    </div>
                  ))}
